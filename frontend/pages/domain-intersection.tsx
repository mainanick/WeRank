...
type T = DomainIntersectionResponse["tasks"][0]["result"][0]["items"][0];
export default function Page() {
  ...
  const [formState, setFormState] = useState<DomainIntersectionRequest>({
    target1: "",
    target2: "",
    location_name: "Kenya",
  ...
  const [results, setResults] = useState<
    DomainIntersectionResponse | undefined
  >();

  const onSubmit = async (ev: FormEvent<HTMLFormElement>) => {
    ev.preventDefault();
    if (
      formState.target1.length === 0 ||
      formState.target2.length === 0 ||
      formState.location_name === ""
    )
      return;
    const res = await API.domainIntersection({ ...formState });
    setResults(res);
  };
  ...
  return (
    <Layout>
      <div className="mb-4">
        <form id="domainIntersection" onSubmit={onSubmit}>
          <div className="flex w-full max-w-sm items-center">
            <Input
              ...
              onChange={(ev) => {
                setFormState({ ...formState, target1: ev.target.value });
              }}
            />
            <Input
              type="text"
              ...
              className="h-8 border-r-0 rounded-r-none"
              onChange={(ev) => {
                setFormState({ ...formState, target2: ev.target.value });
              }}
            />
            <CountrySelect
              name="location_name"
