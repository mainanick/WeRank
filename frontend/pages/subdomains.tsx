...
import { API, SubdomainRequest, SubdomainResponse } from "@/lib/api";
...
type T = SubdomainResponse["tasks"][0]["result"][0]["items"][0];
export default function Page() {
  // TODO Show Metrics
  // eslint-disable-next-line  @typescript-eslint/no-explicit-any
  const columns: ColumnDef<T, any>[] = [
    {
      header: (ctx) => SortColumn(ctx, "Subdomains"),
  ...
  const [formState, setFormState] = useState<SubdomainRequest>({
    target: "",
    location_name: "Kenya",
    language_name: "English",
  });
  const [results, setResults] = useState<SubdomainResponse | undefined>();

  const onSubmit = async (ev: FormEvent<HTMLFormElement>) => {
    ev.preventDefault();
    if (formState.target.length === 0 || formState.location_name === "") return;
    const res = await API.subdomains({ ...formState });
    setResults(res);
  };
  ...
  return (
    <Layout>
      <div className="mb-4">
        <form id="subdomains" onSubmit={onSubmit}>
